...
const faturamentoDiario = dados.filter(dia => dia.valor > 0);
...
const mediaMensal = faturamentoDiario.reduce((acc, dia) => acc + dia.valor, 0) / faturamentoDiario.length;
const diasAcimaMedia = faturamentoDiario.filter(dia => dia.valor > mediaMensal).length;
